...
contamination evidence. \pending{Both are owed: T9.4 and Phase~7.}
...
manifest entry; targets are unit-tested against hand-computed values; and the
splits are guarded by leakage assertions in CI. \pending{T9.3: per-run artifacts
(resolved config, config hash, git SHA, seeds, environment fingerprint) and one
committed config per experiment, with clean-machine regeneration of
Table~\ref{tab:baselines} asserted in CI.}
