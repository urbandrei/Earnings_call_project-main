...
table, and \texttt{ecvol audit predictions} applies the identity controls to any
prediction file (company overlap and embargo of the split, ticker-only and persistence
floors, same- and other-company prediction swaps, the company share of prediction
variance, company-clustered significance), so the diagnosis of
\S\ref{sec:results-controls} can be run on a model without adopting this pipeline; a clean-machine regeneration of Table~\ref{tab:baselines} is part of the
release checklist.
