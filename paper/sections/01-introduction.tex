...
result existed and are recorded in a public decision log. RQ1, RQ2, and RQ4(a--b)
are answered here; RQ3 and RQ4(c) are the subject of the reproduction and
post-cutoff studies that this draft still owes (marked \pending{like this}).
...
\begin{enumerate}
  ...
  \item \textbf{A reproduction study} of the five publicly runnable models in this
    lineage, each re-run on its own original data under the new controls, together
    with a code-availability audit of the fifteen that cannot be run
    \pending{Phase 6R; Table~\ref{tab:repro}}.
  \item \textbf{Post-cutoff evaluation}: the audio ladder re-run on clean 2025
    recordings, and the frozen pipeline evaluated on calls held after the
    knowledge cutoff of every model used \pending{Phase 7 and T9.4}.
\end{enumerate}
