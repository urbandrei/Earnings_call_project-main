...
transcripts but no raw audio. \textbf{Earnings25} \citep{jiang2026earnings25} (CC-BY-4.0; 514 S\&P~500 calls, 498\,h of audio, held July--December 2025 with
97\% in October--December) is post-cutoff for every model we use and carries
webcast MP3 audio at 64\,kbps for 281 calls and $\le$24\,kbps for 232 (one at
32\,kbps); it is
...
cross-checked against Tiingo on a 50-ticker random sample (47 pass, three flagged
and documented, minimum adjusted-return correlation 0.976); delisted or unmatched tickers produce an explicit coverage report rather
...
by up to 8\% on FinCall and up to 26\% on MAEC at $\tau{=}3$ once the reaction
session enters the post window, while the HAR-RV $R^2_{\mathrm{OOS}}$ moves by at
most 0.04 on FinCall and 0.11 on MAEC and Earnings25; the target correlation
between anchors on Earnings25 is 0.81--0.97 for $v_{\mathrm{post}}$ and 0.68--0.80
for $\Delta v$ across horizons. The learned ridge heads are more anchor-sensitive
than HAR-RV (\S\ref{sec:results-lookahead}). Whether to re-baseline on the measured
