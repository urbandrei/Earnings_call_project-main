...
| pending (user) | EC headline run needs the released KePt-BERT-large embedding pickle (`text_embedding`, Drive `1F83bjiJKEpq_MYrc0lzQb9rOLgooz-5E`): **corrected 2026-09-21:** that id is a Drive *folder* (README: `drive/folders/…`), and opened as one it is empty — the released EC embeddings are unavailable upstream; the pre-training data folder `1Xe7Y64F04X9GkWMOSZmSmpV1f5I1ySgM` is populated — HANDOFF 2026-09-21 |
...
| KeFVP, authors' script (MAEC-15 / MAEC-16; 3 repeats) | T6R.3 full test set: 0.419 / 0.185 / 0.123 / 0.086 and 0.442 / 0.288 / 0.363 / 0.188 | MAEC-15 0.441 / 0.230 / 0.263 / 0.098 (train 535 → 272); MAEC-16 0.328 / 0.173 / 0.159 / 0.089 (train 980 → 295) — confounded with training size and the re-carved dev set | MAEC-15 0.458 / 0.199 / 0.150 / 0.113 vs 0.470 / 0.192 / 0.145 / 0.114 (48 calls); MAEC-16 0.378 / 0.241 / 0.312 / 0.178 vs 0.416 / 0.248 / 0.347 / 0.186 (92 calls) | no ticker-disjoint penalty (price-dominated model); embargo rows not interpretable as an embargo effect |
...
Persistence on MAEC uses `past_{30−τ}`: the shipped MAEC files number pre-call windows backwards (`past_29` = 1 session, `past_0` = 30; variances match `future_1` / `future_30`), unlike EC's forward `past_τ`. KeFVP runs load a per-dataset subset of the embedding pickle (same vectors) after two low-memory kills. Paper: Table 6R-C (`paper/tables/repro_controls.tex`), §6 "The reproduced models under our controls".

**HTML shuffle control (2026-09-21, runs 20260921T051957Z text / 20260921T063746Z text+audio; `--shuffle`).** One trained model per seed, test transcripts substituted, scored on the test calls whose company has another EC call (97 / 86 / 92 of 112 / 101 / 109). Code split, text: real 0.683 / 0.348 / 0.262 / 0.165 → same company 0.693 / 0.361 / 0.269 / 0.169 (+1–4%) → different company 0.745 / 0.386 / 0.318 / 0.203 (+9–23%); text+audio same company ≤ +3%, different +16–26%. 68% (code) / 64% (embargoed) of same-company substitutes are training calls; under ticker-disjoint 0% are and the same-company swap is still ≤ 1%, the different-company swap +1–5% (text) / +5–17% (text+audio). Anchor rows unchanged (40/40). Also noted: the shipped published split lists `AbbVie Inc._20170728` twice in train (kept as shipped).
