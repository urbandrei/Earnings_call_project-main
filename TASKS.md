...
- **Subtasks (threads; order = certainty):**
  ...
  - **KeFVP** (authors' `final_series_infer.py`, path/Windows patches; audio branch is commented out upstream, so text+price only is the faithful setting):
    - [!] K1 obtain the released EC KePt embedding pickle (Drive `1F83bjiJKEpq_MYrc0lzQb9rOLgooz-5E`): `gdown`, `curl` and browser-cookie routes all refused; the file opens in the user's browser → **user download, HANDOFF 2026-09-09** — **corrected 2026-09-21: the id is a folder and it is empty; not a user task any more, needs a choice between regenerated embeddings / KePt re-training / reason code (HANDOFF 2026-09-21)**
  ...
  - **SCSS** (authors' TSLib-style `run.py`):
    ...
    - [x] S4 (2026-09-16, run 20260916T163850Z, `ecvol reproduce scss-aug`) Aug_PEV / Aug_STPEV on EC + MAEC — the notebook's own functions run verbatim (not re-implemented); **48/48 cells exact at their 3-decimal rounding**, published values parsed from the notebook's stored outputs; Aug_STPEV (training-free) within 0.05 of KeFVP's published MSE on EC/MAEC-15, below it at τ≥7 on MAEC-16; Aug_PEV uses test-ticker identity (mean of their per-ticker means); τ=30 history windows reaching the test period 4/2195 (EC), 8/3192 (MAEC-15) → vs 0.367/0.296 (EC), 0.283/0.225 (MAEC15), 0.229/0.247 (MAEC16)
...
### T6R.4 Controls sprint — the five reproduced models under embargoed + ticker-disjoint splits — `[x]` *(opened 2026-09-16, done 2026-09-17; scope + protocol DECISIONS 2026-09-16)*
...
- **Subtasks (cheap first):**
  - [x] C1 HTML faithful (2026-09-16, runs 20260916T200408Z text / 20260916T212618Z text+audio; seeds 0–2): anchor rows identical to T6R.3 (24/24). **Text: code 0.660/0.336/0.253/0.160 → ticker-disjoint 0.777/0.522/0.393/0.291 (+18/+55/+55/+82%), above persistence at τ=15/30 (0.327/0.188); embargoed 0.726/0.347/0.246/0.178.** Text+audio: code 0.658/0.347/0.244/0.165 → ticker-disjoint 0.789/0.500/0.394/0.281 (+20/+44/+61/+70%, above persistence τ≥15); embargoed 0.684/0.345/0.232/0.186. Seed std ≤0.04. Caveat: the `code` anchor is a random split, so the controlled test sets differ from it (n 112/101/109) — read deltas with each row's persistence
  - [x] C2 Sawhney port (2026-09-16, run 20260916T190656Z): anchor rows byte-equal to T6R.3 (124/124). **Val-tuned 3-way ensemble — their split 0.686/0.324/0.187/0.117, embargoed 0.719/0.306/0.184/0.107, ticker-disjoint 0.775/0.416/0.263/0.161 (+13/+28/+41/+38%)**; seed std ≤0.006. Finance SVR alone 0.712/0.370/0.218/0.154 → embargoed 0.748/0.380/0.225/0.150 → ticker-disjoint 0.700/0.365/0.256/0.158, so under ticker-disjoint the ensemble is *worse* than its own finance branch at τ≥7. Test-tuned (theirs) 0.639/0.298/0.175/0.102 → 0.681/0.304/0.183/0.102 → 0.690/0.362/0.252/0.158. Test sizes 110/100/109. Reading: the embargo barely matters; removing same-ticker history costs 13–41%
  - [x] C3 SCSS TSMixer/TMLP (2026-09-16, run 20260916T195449Z; authors' run.py, seed 2021, 19 rolling quarters): **TMLP (text) ticker-disjoint vs paired anchor_heldout 0.660/0.354/0.280/0.267 vs 0.591/0.288/0.224/0.211 (+12/+23/+25/+27%), worse in 16/17/17/18 of 19 quarters; above persistence at τ=15/30 (0.252/0.224)**. TSMixer (price only) 0.556/0.254/0.194/0.233 vs 0.519/0.241/0.206/0.252 — mixed, worse in 14/13/8/10 of 19. Embargoed (full 90-call test) ≈ neutral for both: TSMixer 0.570/0.251/0.205/0.232 vs 0.561/0.252/0.210/0.257, TMLP 0.585/0.287/0.216/0.208 vs 0.608/0.285/0.215/0.202. Held-out third = 30 test calls/quarter
  - [x] C4 DialogueGAT port (2026-09-16, run 20260917T003123Z; per-year protocol, seed 1234, single seed): anchor re-run within 0.0014 of T6R.3 (PyG scatter non-determinism — not bit-reproducible, unlike the others). **No consistent ticker-disjoint penalty:** vs paired anchor_heldout, 2019 +15/+7/+18/+8%, 2021 ≈0 (0.726/0.402/0.229/0.083 vs 0.720/0.396/0.227/0.084), 2020 mixed (+28/−22/−36/+48%; the anchor itself stops at epoch 0 there). Embargoed vs their: 2019 +11/+14/+10/+23%, 2021 −7/−6/+52/0%, 2020 τ=30 blows up (1.187). Held-out test sets 42–60 calls/year. Reading: inconclusive — noisy, one seed; the port's speaker nodes are role-typed (no named-speaker identity channel), which may matter
  - [x] C5 KeFVP MAEC-15/16 (2026-09-17, runs 20260917T053924Z / 20260917T133556Z; authors' script, 3 repeats): **ticker-disjoint ≈ paired anchor on both** — MAEC-15 0.458/0.199/0.150/0.113 vs 0.470/0.192/0.145/0.114 (48 test calls), MAEC-16 0.378/0.241/0.312/0.178 vs 0.416/0.248/0.347/0.186 (92). Embargoed vs the full T6R.3 anchor: MAEC-15 0.441/0.230/0.263/0.098 vs 0.419/0.185/0.123/0.086 with train 535→272; MAEC-16 0.328/0.173/0.159/0.089 vs 0.442/0.288/0.363/0.188 with train 980→295 — *better*, because the embargo re-carves a chronological dev set and the anchor's τ≥15 cells sit above persistence; confounded with training size and model selection, not read as an embargo effect. Two low-memory kills (fixed: per-dataset embedding subset) and one persistence-column bug (MAEC `past_k` spans 30−k sessions) along the way
  - [x] C6 Paper (2026-09-17): Table 6R-C `paper/tables/repro_controls.tex` + §6 "The reproduced models under our controls"; prose bounds re-checked against the CSVs (three over-claims corrected before commit); 30 pp, 0 undefined refs
...
- **Notes:** DECISIONS 2026-08-23 §9; design + acceptance amendment DECISIONS 2026-08-26 (later). Before: `artifacts/` held 8 diagnostic files and no run payload, `configs/` only `example.yaml`, `tracking.write_run` never called. After: `ecvol report` renders only provenance-verified CSVs; `ecvol runs verify` checks all eleven. ~~Still `backfill`: Tables 2/3/4, controls, audio, fusion~~ → **upgraded 2026-09-16** (commit 63bf39e): `evaluate-text`, `controls`, `evaluate-audio`, `evaluate-fusion`, `grid` re-run live, every output byte-identical to the committed CSV (MLP heads are seed-*unstable across seeds* but deterministic per seed on this machine); `ecvol runs verify` 29 files, 0 backfill. The re-run exposed a latent `grid` bug (T9.1 calendar rows of Table 1 entering Table 4 as duplicates) — fixed with a test (45a47e7). Curated `paper/tables/*.tex` remain hand-copied cells → T8.1.
...
- **Subtasks:**
  ...
  - [x] `ecvol audit predictions` (2026-09-23, DECISIONS 2026-09-23) — the §7.3 controls on any prediction frame (split integrity, ticker-only/persistence/train-mean floors, seen/unseen, same-/other-company prediction swaps, identity share, DM + clustered CI); `docs/audit_predictions.md`; demo on SCSS TMLP's own predictions in `data/audits/demo/` (manifested); 5 tests
