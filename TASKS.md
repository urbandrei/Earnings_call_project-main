...
### T3.2 Frozen text features — `[~]` *(impl + benchmark done 2026-06-19; full-corpus extraction pending user go)*
...
- **Subtasks:**
  ...
  - [ ] **Full-corpus extraction** (FinCall 196,233 chunks / MAEC 394,280) — pending user go-ahead after the benchmark.
- **Notes:** model picks BGE-M3 + ProsusAI/finbert, fp32 + deterministic kernels (DECISIONS 2026-06-19). **Benchmark (50 FinCall calls = 3,130 chunks, fp32, incl. one-time model load → conservative):** BGE-M3 37 ch/s, FinBERT 90 ch/s → full-corpus ETA FinCall ~2 h, MAEC ~4 h; content-hash cache makes the run resumable. **Acceptance status:** cache bit-identical re-run = unit-tested (`encode_with_cache`); throughput + ETA recorded (here) — the only remaining piece is running the full extraction. 8 new tests (torch-free, CI-safe); 182 green, ruff clean. Journal: 2026-06-19 T3.2 entry.
