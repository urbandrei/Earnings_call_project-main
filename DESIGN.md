...
Let `P_t` be the **adjusted close** on trading day `t`, with `t = 0` the last trading day **before** the call's information is public, defined by the **after-hours rule**: if the call begins at or after 16:00 ET (or timestamp is missing but the call is marked "after market close"), day `t = 1` is the next trading day; if before 09:30 ET, the call's own date is day 1; intraday calls (rare) are treated as after-hours and flagged. *(Amended 2026-09-03 — DECISIONS 2026-09-03: where a measured timestamp exists, the boundary is the **earnings-release** time — the acceptance time of the Item 2.02 8-K — not the call start, with provenance tiers `edgar_8k` › `earnings25_metadata` › `dec_flag` › `assumed_after_hours` recorded per call in `coverage/{dataset}_timing.csv`; intraday releases are more common than "rare" — Earnings25 measures 29% of call starts intraday — and remain after-hours-and-flagged until a rule change is decided on the T9.2 sensitivity evidence.)* Daily simple return: `r_t = (P_t − P_{t−1}) / P_{t−1}`.
...
.                            # project root
  ...
  artifacts/                 # run outputs; runs/<run_id>/run.json manifests ARE committed (T9.3), heavier payloads are not
```
...
- **Configs:** YAML validated by pydantic schemas. Deliberately **not Hydra** — one researcher; debuggability beats composability. Every experiment = one config file under `configs/`, committed. *(Amended 2026-08-26, DECISIONS 2026-08-26 T9.3 entry: the unit is the **result-producing command** — `configs/<command>.yaml` (`CommandConfig`) for `evaluate`, `evaluate-text`, `controls`, `evaluate-audio`, `evaluate-fusion`, `grid`; CLI flags override, the manifest records the resolved values. `ExperimentConfig` remains the schema for single-model runs.)*
- **Tracking:** every run writes `artifacts/runs/<run_id>/` containing resolved config, config hash, git SHA, seed list, environment fingerprint, and metrics parquet. Deliberately **not DVC/wandb** for v1 — JSON manifests + parquet are sufficient and have no service dependency (wandb may be added later via Decision Log). *(Amended 2026-08-26: for the result-producing commands the run artifact is `run.json` — resolved `CommandConfig` + hash, git SHA/dirty flag, env fingerprint, seeds, and the SHA-256 of every `data/results/*.csv` the command wrote. Manifests are committed; `ecvol report` and CI (`tests/test_runs.py`) refuse any result CSV whose bytes match no manifest. Manifests written over pre-existing files carry `provenance: backfill` until the producing command is re-run.)*
...
- [x] One config file per experiment, committed; config hash + git SHA in every artifact. *(2026-08-26, T9.3: per result-producing command — see §8.2.)*
...
- [ ] All paper numbers generated by `ecvol report` from artifacts — no hand-transcribed results. *(2026-08-26: `ecvol report` now renders only provenance-verified CSVs; the curated `paper/tables/*.tex` still copy cells by hand — closing that is T8.1.)*
